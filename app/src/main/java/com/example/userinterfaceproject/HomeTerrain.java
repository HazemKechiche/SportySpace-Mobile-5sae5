...
public class HomeTerrain extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        buttonPlus.setOnClickListener(v -> {
            Intent intent = new Intent(getActivity(), Publish2.class);
            startActivity(intent);
        });

        // Locate the "retour" button and set the click listener
        ImageView retourButton = view.findViewById(R.id.backArrowButtonTerrain);
        retourButton.setOnClickListener(v -> {
            Intent intent = new Intent(getActivity(), Acceuil.class); // Replace `AcceuilActivity.class` with the actual class of your accueil activity
            startActivity(intent);
        });
        return view;
    }
